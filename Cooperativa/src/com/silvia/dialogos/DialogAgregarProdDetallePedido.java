...
public class DialogAgregarProdDetallePedido extends DialogFragment implements OnItemClickListener, OnClickListener{
	...
	public DialogDetallePedido dialog_detalle_pedido;
	
	public DialogAgregarProdDetallePedido(DialogDetallePedido dialog_detalle_pedido){
	...
	private void agregarDetalleVenta() {
		...
		if(!txtCantidad.equals("")){
			...
			if(!txtCostoEntrega.equals("")){
				...
				detalleP = new DetallePedido(generarIdDetallePedido(), dialog_detalle_pedido.pedido.getIdpedido(), producto.getIdprod(), 
												cantidad, costo_entrega, costo);
				double costo_total_anterior = dialog_detalle_pedido.pedido.getCosto_total();
				dialog_detalle_pedido.pedido.setCosto_total(costo_total_anterior+costo);
			}else{
		...
		if(detalleP!=null){
			if(regitrarDetallePedido(detalleP)){
				dialog_detalle_pedido.editarDatosPedido(dialog_detalle_pedido.pedido);
				dialog_detalle_pedido.my_adapter.add(detalleP);
				dialog_detalle_pedido.my_adapter.notifyDataSetChanged();
				
				dialog_detalle_pedido.tvCostoTotal.setText(new StringBuilder().append(dialog_detalle_pedido.pedido.getCosto_total()).append(" Bs."));
				limpiarCampos();
				Toast.makeText(getActivity(), "Producto agregado a la lista de pedido", Toast.LENGTH_SHORT).show();
			}else{
				Toast.makeText(getActivity(), "No se pudo agregar mas producto al pedido", Toast.LENGTH_LONG).show();
			}
			
		}		
	}
	
	public boolean regitrarDetallePedido(DetallePedido detalle){
		DBDuraznillo db = new DBDuraznillo(getActivity());
		try {
			db.abrirDB();
			if(db.insertarDetallePedido(detalle)){
				return true;
			}
			db.cerrarDB();
		} catch (Exception e) {
